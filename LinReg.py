...
def linreg_all(step,abs_err,name):
...
def linreg_ind(step,abs_err):
    ...
    theta = np.linalg.inv(X_transpose.dot(data_X)).dot(X_transpose).dot(data_y)
    #Plotting the linear regression:
    plt.plot(data_X[:,1],data_X.dot(theta),'-',label='Linear Regression',color='b')
    #Plotting raw data to compare:
    plt.scatter(data_x,data_y,s=8,marker = 'x',color = 'r',label = 'Raw Data')
    #plt.xlabel('X')
    #plt.ylabel('Y')
    plt.autoscale()
    #plt.show()
